...
- `resolveFollowUpRequest` can safely expand `more` and natural variants such as `make it more` to the last recorded request, resolve `less` and natural variants such as `make it less` plus explicit revert wording against version ancestry, resolve `undo` against explicit active-ref history, and branch `try another version` from the prior baseline when the required session provenance exists.
