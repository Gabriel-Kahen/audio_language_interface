...
- Clarification state is only cleared automatically when a later request is actually treated as `followUpResolution.source = "clarification_answer"` or when a revert-style request abandons the pending path; unrelated direct requests do not silently consume the pending clarification.
