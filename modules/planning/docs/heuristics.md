...
- `less harsh`, `tame the harshness`, `take out some harshness`, `smoother`, non-regional `softer` -> `notch_filter` centered on the analysis harshness annotation midpoint, or `3750 Hz` fallback
...
- `control peaks`, `catch peaks`, `catch the peaks`, `keep the peaks in check`, `limit the peaks`, `limiter` -> conservative `limiter` settings with explicit `ceiling_dbtp`, `release_ms`, `lookahead_ms`, and no added limiter input gain by default
...
- `quieter`, `turn it down`, `turn the level down` -> conservative negative `gain` step
