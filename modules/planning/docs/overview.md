...
- maps conservative `more controlled` language to `compressor`, maps explicit louder-and-controlled language to measured compressor-plus-loudness staging when dynamics still have room to tighten safely, maps louder-and-controlled wording on already-controlled sources to a small peak-limited input-gain move, keeps explicit normalize-louder-controlled wording on a peak-protected normalize path for already-controlled sources, maps explicit peak-control language to `limiter`, refuses pure control wording on sources that already measure as tightly controlled, and records an explicit constraint when redundant control is skipped while a companion tonal objective proceeds
...
- supports the safe low-band tonal compound `warmer` plus `clean up the low mids` as a low-mid `parametric_eq` cut followed by a low-shelf warmth boost
